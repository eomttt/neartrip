...
export async function respondToApi(request: Request, action: () => unknown | Promise<unknown>) {
  return traceApiRequest(request, async (traceId) => {
    ...
    if (!rate.allowed)
      return Response.json(
        { error: '요청이 너무 많습니다. 잠시 후 다시 시도해주세요.', traceId },
    ...
    try {
      ...
      if (error instanceof z.ZodError)
        return Response.json(
          { error: '요청 또는 지도 응답의 형식이 올바르지 않습니다.', traceId },
          { status: 400, headers },
        );
      if (error instanceof ProviderError || error instanceof RequestError)
        return Response.json({ error: error.message, traceId }, { status: error.status, headers });
      if (error instanceof SyntaxError)
        return Response.json(
          { error: '요청 내용을 읽을 수 없습니다.', traceId },
          { status: 400, headers },
        );
      return Response.json(
        { error: '요청을 완료하지 못했습니다. 다시 시도해주세요.', traceId },
        { status: 500, headers },
      );
    }
  });
...
export async function readPlanBody(request: Request): Promise<unknown> {
  ...
  try {
    while (true) {
      ...
      if (size > 20 * 1_024) {
        recordRequestBody({ captured: false, reason: 'body_too_large', bytesRead: size });
        await reader.cancel();
    ...
    text += decoder.decode();
    recordRequestBody({ captured: false, reason: 'invalid_json', bytesRead: size });
