...
it('혼잡도에 구역과 한국 갱신 시각을 표시하고 개별 매장 정보와 구분한다', () => {
  ...
  expect(screen.getByLabelText('주변 여유 안내').closest('details')?.open).toBe(false);
  expect(screen.getByText(/성수카페거리/)).toBeTruthy();
  expect(screen.getByText(/12:55/)).toBeTruthy();
  fireEvent.click(screen.getByLabelText('주변 여유 안내'));
  expect(screen.getByLabelText('주변 여유 안내').closest('details')?.open).toBe(true);
  expect(screen.getByText(/개별 매장의 빈자리나 대기 인원과 달라요/)).toBeTruthy();
  expect(screen.getByRole('link', { name: '출처: 서울 열린데이터광장' })).toBeTruthy();
  fireEvent.click(screen.getByLabelText('주변 여유 안내'));
  expect(screen.getByLabelText('주변 여유 안내').closest('details')?.open).toBe(false);
});
it('화면에 남아 있는 값도 오래되면 여유 등급을 숨긴다', () => {
  vi.useFakeTimers();
  vi.setSystemTime(new Date('2026-09-11T05:00:00Z'));
  const { container } = render(<CrowdingBadge crowding={value} />);
...
it('예시 등급과 대체 데이터는 표시를 구분한다', () => {
  ...
  fireEvent.click(screen.getByLabelText('주변 예시 여유 안내'));
  expect(screen.getByText('서울시 대체 데이터 기준입니다.')).toBeTruthy();
});
const hiddenStates: Crowding['state'][] = ['unsupported', 'not_configured', 'unavailable', 'stale'];
